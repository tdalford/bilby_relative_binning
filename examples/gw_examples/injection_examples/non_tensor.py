...
Extra requirements
==================
- nestle: a pure-python nested sampling package, available from pypi

Typical run time: ~ 10 minutes
"""
import bilby
import numpy as np
...
priors["ra"] = bilby.core.prior.Uniform(
    minimum=0, maximum=2 * np.pi, latex_label="$\\alpha$", boundary="reflective"
)
priors["dec"] = bilby.core.prior.Cosine(latex_label="$\\delta$")
priors["epsilon"] = bilby.core.prior.Uniform(
    minimum=0, maximum=1, latex_label="$\\epsilon$"
)
